...
export default function StudentDashboardLayout({ children }) {
  ...
  // Sidebar Links
  const menuItems = [
    { name: 'Dashboard Home', path: '/student-dashboard', icon: LayoutDashboard },
    { name: 'My Profile', path: '/student-dashboard/profile', icon: User },
    { name: 'My Courses', path: '/student-dashboard/courses', icon: BookOpen },
    { name: 'Saved & Applied Jobs', path: '/student-dashboard/jobs', icon: Briefcase },
    { name: 'Verifiable Certificates', path: '/student-dashboard/certificates', icon: Award },
    { name: 'Notifications', path: '/student-dashboard/notifications', icon: Bell },
    { name: 'Settings & Privacy', path: '/student-dashboard/settings', icon: Settings },
  ];

  // Breadcrumb path builders
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 flex transition-colors duration-200">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden bg-gray-900/40 backdrop-blur-sm">
          <div className="w-64 bg-white dark:bg-gray-900 h-full flex flex-col p-6 border-r border-gray-100 dark:border-gray-800 shadow-xl transition-transform duration-200">
            <div className="flex items-center justify-between mb-8">
              <Link to="/" className="flex items-center space-x-2">
                <div className="bg-primary p-2 rounded-xl text-white">
                  <Sparkles className="w-5 h-5" />
                </div>
                <span className="text-lg font-black text-gray-900 dark:text-white">ElevateX</span>
              </Link>
      ...
      <aside className="hidden md:flex md:w-64 flex-col bg-white dark:bg-gray-900 p-6 border-r border-gray-100 dark:border-gray-800 shrink-0 sticky top-0 h-screen transition-colors">
        ...
        {/* User Account info and Sign Out block */}
        <div className="pt-6 border-t border-gray-100 dark:border-gray-800">
          <button
            onClick={handleLogout}
            className="w-full flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all"
      ...
      <div className="flex-grow flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 px-6 flex items-center justify-between sticky top-0 z-40 transition-colors">
          ...
          {/* Breadcrumb / Menu trigger on Mobile */}
          <div className="flex items-center space-x-3">
            ...
            <div className="hidden sm:flex items-center space-x-1.5 text-xs font-semibold text-gray-500 dark:text-gray-400">
              <Link to="/" className="hover:text-primary">ElevateX</Link>
